...
{
    public sealed class EditorPane : WindowPane, IOleCommandTarget, IVsPersistDocData, IPersistFileFormat, IVsToolboxUser
    {
        static OleDataObject? toolboxData = null;

        readonly Logger logger;
        readonly EditorControl editorControl;
        ...
        internal EditorPane(Logger logger) : base(null)
        {
            ...
            editorControl = new (logger);
            editorControl.AllowDrop = true;
            editorControl.HideSelection = false;
            editorControl.TabIndex = 0;
            editorControl.Text = string.Empty;
            editorControl.Name = nameof(EditorPane);

            editorControl.DragEnter += OnDragEnter;
            editorControl.DragDrop += OnDragDrop;
            editorControl.TextChanged += OnTextChange;
        }
        ...
        public override IWin32Window? Window => editorControl;
        ...
        {
            _ = logger.LogAsync(DebugContext.EditorPane, $"Executing group {pguidCmdGroup}, cmd {nCmdID}");
        ...
        {
            ppszFormatList = "WiX UI Designer (*.wxs)\n*.wxs\n\n";
            return VSConstants.S_OK;
        }
        ...
        /// <returns>S_OK if the function succeeds.</returns>
        int IPersistFileFormat.SaveCompleted(string pszFilename) => noScribbleMode ? VSConstants.S_FALSE : VSConstants.S_OK;
        ...
        {
            _ = logger.LogAsync(DebugContext.EditorPane, $"Initializing new file with format index {nFormatIndex}");
            isDirty = false;
            return VSConstants.S_OK;
        }
        ...
        /// </summary>
        /// <param name="path">Pointer to the full path name of the file to load.</param>
        /// <param name="mode">file format mode.</param>
        /// <param name="readOnly">determines if the file should be opened as read only.</param>
        /// <returns>S_OK if the method succeeds.</returns>
        int IPersistFileFormat.Load(string path, uint mode, int readOnly)
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            _ = logger.LogAsync(DebugContext.EditorPane, $"Loading file {path}");
        ...
        /// </summary>
        /// <param name="pszFilename">Pointer to the file name. If the path parameter is a null reference 
        ...
        /// VSSAVE_SaveAs      - Prompts the User for a path and saves the file to the file specified.
        /// VSSAVE_SaveCopyAs  - Prompts the user for a path and saves a copy of the file with a name specified.
...
break;
                            ...
                            default:
                                throw new COMException("Error WixUiDesigner 1");
                        }
                        break;
                    }
                ...
                case VSSAVEFLAGS.VSSAVE_SaveCopyAs:
                    {
                        ...
                        if (string.Compare(".wxs", System.IO.Path.GetExtension(fileName), true, CultureInfo.CurrentCulture) != 0)
                            fileName += ".wxs";
                        // Call the shell to do the save for us
                        IVsUIShell uiShell = (IVsUIShell)GetService(typeof(SVsUIShell));
                        hr = uiShell.SaveDocDataToFile(dwSave, this, fileName, out pbstrMkDocumentNew, out pfSaveCanceled);
                ...
                default:
                    throw new ArgumentException("Error WixUiDesigner 1");
            };
